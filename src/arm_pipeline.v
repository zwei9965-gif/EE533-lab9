...
module arm_pipeline (
    ...
    // =========================================================
    // CPSR flags (per-thread, flattened for XST compatibility)
    // =========================================================
    ...
    // Forward declarations (regfile outputs, used by barrel shifter below)
    ...
    // Pre-decode register addresses from IF stage (one cycle before ID)
    // BRAM sync read: address at cycle N → data at cycle N+1
    // Present addresses during IF so data is ready during ID
    wire pre_is_bx = (if_instr[27:4] == 24'h12FFF1);
    wire pre_is_ls = (if_instr[27:26] == 2'b01);
    // Rn address: BX uses Rm[3:0], others use [19:16]
    wire [3:0] pre_rn_addr = pre_is_bx ? if_instr[3:0] : if_instr[19:16];
    // R1 address: LS non-reg uses Rd[15:12], others use Rm[3:0]
    wire [3:0] pre_r1_addr = (pre_is_ls && !if_instr[25]) ? if_instr[15:12] : if_instr[3:0];

    arm_regfile regfile_inst (
        .clk       (clk),
        .rst       (rst),
        .thread_id (if_thread),    // IF stage thread (one cycle early)
    ...
    // MUX per-thread CPSR flags based on idex_thread
